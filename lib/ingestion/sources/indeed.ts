 ...
 *
 * Uses JSearch (OpenWeb Ninja) with Indeed-specific queries.
...
export class IndeedSource implements OpportunitySource {
  ...
  async fetch(): Promise<RawOpportunity[]> {
    ...
    if (!endpoint) {
      console.warn("[Indeed] No working JSearch endpoint — skipping.");
      return [];
    }
    ...
    if (!tryReserveJSearchRequests(INDEED_QUERIES.length)) {
      console.warn(
        `[Indeed] Request budget exhausted (${getJSearchRequestsReserved()}) — skipping this run.`
      );
      return [];
    }
